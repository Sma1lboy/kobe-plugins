#!/usr/bin/env node
// kobe.browser.open <url> — remember the URL and open the pane WITHOUT
// the URL prompt (one-shot skip marker).
//   kobe plugin action invoke kobe.browser.open https://localhost:5173
...
if (!raw) {
  console.error("usage: kobe plugin action invoke kobe.browser.open <url>")
  process.exit(2)
}
...
const res = await openPane("kobe.browser.browse")
